...
VALUES ($1, $2, $3)
RETURNING id, configuration_version_id, status
`
...
func (q *Queries) CreateTFERun(ctx context.Context, arg CreateTFERunParams) (TFERun, error) {
	...
	var i TFERun
	err := row.Scan(&i.ID, &i.ConfigurationVersionID, &i.Status)
	return i, err
}
...
SELECT id, configuration_version_id, status
FROM tfe_run
WHERE id = $1
`
...
func (q *Queries) ListTFERuns(ctx context.Context) ([]TFERun, error) {
	...
	for rows.Next() {
		var i TFERun
		if err := rows.Scan(&i.ID, &i.ConfigurationVersionID, &i.Status); err != nil {
			return nil, err
		}
